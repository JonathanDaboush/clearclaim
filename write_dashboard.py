              ...
              {billingMetrics ? (
                <>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-primary">${String.fromCharCode(123)}(billingMetrics.total_paid / 100).toFixed(2){String.fromCharCode(125)}</span>
                  </div>
                  {billingWarning && (
...
# Fix the dollar sign expression that got mangled: replace placeholder
content = content.replace(
    '${String.fromCharCode(123)}(billingMetrics.total_paid / 100).toFixed(2){String.fromCharCode(125)}',
    '${(billingMetrics.total_paid / 100).toFixed(2)}'
)

with open(r'c:\Users\USER\Documents\clearclaim\frontend\src\pages\DashboardPage.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print('Written OK')
