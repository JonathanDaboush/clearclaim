...
def verify_totp(user_id, code):
    ...
    return auth_service.verify_totp(user_id, code)
...
def verify_new_device(user_id, device_id):
...
def start_identity_verification(user_id):
    """Controller for starting identity verification."""
    return auth_service.start_identity_verification(user_id)

def store_identity_verification_result(user_id, provider, status):
...
def check_identity_verification_status(user_id):
